...
uint8_t Serial_RxData2;		//定义串口接收的数据变量
uint8_t Serial_RxFlag2;		//定义串口接收的标志位变量
...
{
	return Serial_RxData2;			//返回接收的数据变量
}
...
void USART2_IRQHandler(void)
{
	...
	{
		Serial_RxData2 = USART_ReceiveData(USART2);				//读取数据寄存器，存放在接收的数据变量
		Serial_RxFlag2 = 1;										//置接收标志位变量为1
